...
CENTRAL_VALLEY_CAMERAS = [
    cam for cam in TRAFFIC_CAMERAS if cam["segment"] == "Central Valley"
]
...
async def extract_camera_image(http_client, camera):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Successfully fetched image for {camera['name']} ({len(image_bytes)} bytes)"
            )
            return {
                "camera": camera,
                "image": base64_image,
            ...
            print(
                f"Failed to fetch image for {camera['name']}: HTTP {response.status_code}"
            )
            return {
                "camera": camera,
                "image": None,
...
async def generate_traffic_summary(mountain_analyses, valley_analyses):
    ...
    try:
        ...
        sierra_analyses = [
            a
            for a in mountain_analyses
            if a["camera"]["segment"] == "Sierra" and a["success"]
        ]
        foothills_analyses = [
            a
            for a in mountain_analyses
            if a["camera"]["segment"] == "Foothills" and a["success"]
        ]
        sacramento_analyses = [
            a
            for a in valley_analyses
            if a["camera"]["segment"] == "Valley" and a["success"]
        ]
        central_analyses = [
            a
            for a in valley_analyses
            if a["camera"]["segment"] == "Central Valley" and a["success"]
        ]
        bay_analyses = [
            a
            for a in valley_analyses
            if a["camera"]["segment"] == "Bay Area" and a["success"]
        ]

        # Format each segment's analysis text
        sierra_text = "\n".join(
            [f"{a['camera']['name']}: {a['analysis']}" for a in sierra_analyses]
        )
        foothills_text = "\n".join(
            [f"{a['camera']['name']}: {a['analysis']}" for a in foothills_analyses]
        )
        sacramento_text = "\n".join(
            [f"{a['camera']['name']}: {a['analysis']}" for a in sacramento_analyses]
        )
        central_text = "\n".join(
            [f"{a['camera']['name']}: {a['analysis']}" for a in central_analyses]
        )
        bay_text = "\n".join(
            [f"{a['camera']['name']}: {a['analysis']}" for a in bay_analyses]
        )

        messages = [
            {
                "role": "user",
                "content": f"Here are the current conditions in the Sierra Nevada mountains on US-50 from South Lake Tahoe:\n{sierra_text}",
            },
            {
                "role": "user",
                "content": f"Conditions in the foothills near Placerville:\n{foothills_text}",
            },
            {
                "role": "user",
                "content": f"Conditions in Sacramento and Davis where US-50 meets I-80:\n{sacramento_text}",
            },
            {
                "role": "user",
                "content": f"Conditions in the Central Valley on I-80 (Vacaville, Fairfield):\n{central_text}",
            },
            {
                "role": "user",
                "content": f"Conditions in the Bay Area (Vallejo, Emeryville, San Francisco):\n{bay_text}",
            },
            {
                "role": "user",
...
async def analyze_traffic():
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            mountain_results = await asyncio.gather(
                *[extract_camera_image(client, camera) for camera in MOUNTAIN_CAMERAS]
            )
            valley_results = await asyncio.gather(
                *[extract_camera_image(client, camera) for camera in VALLEY_CAMERAS]
            )

            # Analyze each image with Claude Haiku (concurrently)
            print("Analyzing camera images...")
            mountain_analyses = await asyncio.gather(
                *[analyze_camera_image(result) for result in mountain_results]
            )
            valley_analyses = await asyncio.gather(
                *[analyze_camera_image(result) for result in valley_results]
            )
